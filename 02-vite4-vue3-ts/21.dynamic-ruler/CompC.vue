...
const widthCount = computed(() => {
  return (
    Math.ceil(rect.value.height / (realGapWidth.value * scaleFactor.value)) + 1
  )
})
...
const labelList = computed(() => {
  ...
  for (; index < max; index++) {
    ...
    list.push({
      label,
      style: {
        top: `${num2 + scaleOffsetX.value}px`,
      },
    })
  }
  return list
...
const dList = computed(() => {
  ...
  for (; index < max; index += 5) {
    if (index === 0) {
      list.push(`M0 .5 L18 .5`)
    } else {
      list.push(`M13 ${index + 0.5} L18 ${index + 0.5}`)
    }
  }
  return list.join(' ')
})
const styleBg = computed(() => {
  return {
    ...
    backgroundPosition: `0 ${scaleOffsetX.value}px`,
