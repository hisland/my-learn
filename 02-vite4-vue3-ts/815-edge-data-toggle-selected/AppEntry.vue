...
onMounted(() => {
  graphRef = new Graph({
    ...
    connecting: {
      ...
      validateConnection(obj) {
        if (obj.targetMagnet?.getAttribute('port-group') !== 'groupAction') {
          return false
        }
  ...
  graphRef.on('edge:added', (obj) => {
    console.log('oedge:added bj: ', obj)
    obj.edge.setData({})
  })
  graphRef.on('edge:click', (obj) => {
    console.log('edge:click obj: ', obj)
    if (!obj.edge.data.selected) {
      obj.edge.attr({
        line: {
          stroke: 'red',
          strokeWidth: 3,
    ...
    obj.edge.data.selected = !obj.edge.data.selected
  })
...
function toJSON1() {
  console.log(graphRef.toJSON())
}
</script>
<template>
  <div>
    <button @click="toJSON1">toJSON1</button>
  </div>
