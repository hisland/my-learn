...
const camera = new THREE.PerspectiveCamera(75, windowSizeInfo.aspect, 10, 3000)
...
const frustumSize = getPerspectiveCameraFrustumSize(camera)
...
function getPerspectiveCameraFrustumSize(camera: THREE.PerspectiveCamera) {
  const vFov = (camera.fov * Math.PI) / 180 // 垂直视角（弧度）
  const height = 2 * Math.tan(vFov / 2) * camera.near // 近截面高度
  const width = height * camera.aspect // 近截面宽度
