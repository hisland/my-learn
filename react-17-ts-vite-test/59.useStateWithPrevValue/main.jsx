...
function useStateWithPrevValue(initialState) {
  const [val, setVal] = useState(initialState)
  const ref = useRef()
  useEffect(() => {
    ref.current = val
  }, [val])
  return [val, setVal, ref.current]
}

function Sub1(props) {
  console.log('Sub1 render', props)
  const [num, setNum, oldNum] = useStateWithPrevValue(1)
  const [bar, setBar, oldBar] = useStateWithPrevValue(100)
  function addNum() {
    setNum(num + 1)
  }
  function addBar() {
